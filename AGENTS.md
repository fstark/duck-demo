...
This server exposes **42 MCP tools** organized by tags for client-side filtering.
...
Clients should:
1. Call `list_tools` to get all 42 tools
2. Filter by tags based on agent type:
   - **Sales agent**: `tags=['shared', 'sales']` → 33 tools
...
- `server.py`: Main server (registers all 42 tools)
